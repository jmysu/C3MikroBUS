...
void neopixelOff()
{
    ws2813Write(pinNeopixel, 0,0,0); // Black    
}
/**
 * @brief neopixelBlink
...
{
    tickerMyNeo.detach();
    ws2813Write(pinNeopixel, _neoR, _neoG, _neoB);
...
void neopixelSetupBlink(int msBlink)
{
    ...
    tickerMyNeo.attach_ms(_msBlinkInterval, neopixelBlink);
    log_i("Start blink pin%d @%dms RGB:%d/%d/%d", pinNeopixel, _msBlinkInterval, _neoR,_neoG,_neoB);
}